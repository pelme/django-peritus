...
__all__ = ('SECountySelect', 'SEOrganisationNumberField', 'SEPersonalIdentityNumberField', 'SEPostalCodeField')

class SECountySelect(forms.Select):
    """
    A Select form widget that uses a list of the Swedish counties (län)
    as its choices.
    ...
    def __init__(self, attrs=None):
        ...
        super(SECountySelect, self).__init__(attrs=attrs, choices=COUNTY_CHOICES)
...
class SEOrganisationNumberField(forms.CharField):
    ...
    This fields accepts the same input as SEPersonalIdentityField (for 
    sole proprietorships (enskild firma)). However, co-ordination numbers are
    not accepted.
...
class SEPersonalIdentityNumberField(forms.CharField):
    ...
    + indicates that the person is older than 100 years, which will be
    taken into consideration when the date is validated.
    
    The checksum will be calculated and checked. The birth date is checked
    to be a valid date.

    By default, co-ordination numbers (samordningsnummer) will be accepted.
    To only allow real personal identity numbers, pass the keyword argument
    ...
    def __init__(self, *args, **kwargs):
        self.coordination_number = kwargs.pop('coordination_number', True)
